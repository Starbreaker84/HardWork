...
Тут конечно вложенность if-else удручающая. Применил технику "от обратного" - выходим из метода по мере поступления негативных сценариев, избавляемся от else. Плюс применяем технику вынесения действий в отдельные методы лдя читабельности. Также применили технику тернарного оператора для окончательного возврата значения.
...
```
Классическая проверка вхадящих занчений на null. В этом контексте целесообразно применить Null Object Pattern с использованием Optional.
